...
int main() {
    ...
    Mat img_gray2; //Преобразование изображения в градации серого
    cvtColor(image2, img_gray2, COLOR_BGR2GRAY);
    Mat thresh2; //Применение бинарной пороговой обработки
    threshold(img_gray2, thresh2, 150, 255, THRESH_BINARY);

    vector<vector<Point>> contours3; //Поиск контуров с использованием режима RETR_LIST
    vector<Vec4i> hierarchy3;
    findContours(thresh2, contours3, hierarchy3, RETR_LIST, CHAIN_APPROX_NONE);
    Mat image_copy4 = image2.clone(); //Отрисовка контуров на оригинальном изображении
    drawContours(image_copy4, contours3, -1, Scalar(0, 255, 0), 2);
    imshow("LIST", image_copy4); //Отображение и сохранение результата
    waitKey(0);
